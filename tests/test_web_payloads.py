...
class WebMonitorTest(unittest.TestCase):
    ...
    def test_page_includes_spot_grid_and_dca_controls(self) -> None:
        ...
        self.assertIn("function accountIdentityValue(account, allowAccountId = false)", APP_JS)
        self.assertIn("function exchangeAccountLabel(account", APP_JS)
        self.assertNotIn("function accountIdentitySuffix(account, allowAccountId = false)", APP_JS)
        self.assertNotIn("API #${identity}", APP_JS)
        self.assertIn("option.textContent = exchangeAccountLabel(account);", APP_JS)
