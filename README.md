...
Exchange support is intentionally conservative. Binance, Bybit, Gate.io, HTX, MEXC, KuCoin, Coinbase, and Upbit support guarded post-only limit orders through ccxt. Gate.io, HTX, MEXC, and KuCoin use the single-order path so every placement and cancellation is reconciled individually. Bithumb does not expose post-only support through ccxt, so Bithumb MM with `post_only: true` is blocked before order placement; only set `post_only: false` and `risk.require_post_only: false` for Bithumb if you explicitly accept taker-fill risk. Bithumb also does not support client order ids through ccxt, so its MM orders can only be tracked in memory until the process restarts.
...
The web page exposes runtime controls for the selected account, `enabled`, `side`, `price_mode`, `price_offset_bps`, `unlimited_total`, `slice_mode`, `total_base`, `total_quote`, the min/max base order size range, randomization, `interval_seconds`, `order_ttl_seconds`, `start_price`, and `stop_price`. The account selector includes every configured spot account plus Binance USDM, Bybit, Gate.io, HTX, MEXC, and KuCoin stablecoin linear-swap accounts. Multiple accounts should use distinct `label` values. These page edits affect the running monitor immediately but do not write back to the base JSON config.

Auto Buy/Sell can also execute Binance USDM, Bybit, Gate.io, HTX, MEXC, and KuCoin stablecoin linear perpetuals. Set `instrument_type` to `perpetual` and use a canonical ccxt symbol such as `BTC/USDT:USDT` or `BTC/USDC:USDC`. Supported quote and settlement currencies are `USD`, `USDT`, `USDC`, `FDUSD`, `BUSD`, `TUSD`, `USDP`, `DAI`, `PYUSD`, and `USDE`, subject to the exchange actually listing the market. The configured `quote_rates` must contain the contract quote currency before live risk approval. The UI quantities remain underlying base units and are converted to exchange contracts using `contractSize`. Perpetual tasks require a finite target and configured slice size. `position_effect` defaults to `reduce_only`; every live reduce order is checked against the current matching position and sent with `reduceOnly`. Opening or increasing a position requires `max_position_quote`, a positive `risk.max_derivative_leverage`, and a task `leverage` no higher than that global limit. Gate.io and HTX receive `marginMode` through their leverage endpoint because their ccxt adapters do not expose a separate margin-mode endpoint. Only one-way position mode is supported; hedge-mode accounts must not be used with this task type. MEXC API availability is checked per account and symbol because the venue may suspend API trading for individual pairs.
...
The control page also exposes runtime market setup. `Markets` configures spot arbitrage symbols per account, while `Cash & Carry Pairs` configures spot-vs-contract symbols for basis scanning. Cash & Carry now scans both positive basis (`buy spot + sell contract`) and negative basis (`sell/borrow spot + buy contract`) opportunities. For Binance, Bybit, Gate.io, HTX, MEXC, and KuCoin USDT perpetuals, ccxt symbols usually look like `BTC/USDT:USDT` or `ETH/USDT:USDT`; the spot side remains `BTC/USDT` or `ETH/USDT`. The ACS example config includes `binance-spot`, `binance-swap` (`binanceusdm`), and `bybit-swap`, but does not assume ACS is listed on those venues. Add only the symbols that actually exist on the target exchange.
...
exchange account through the user workspace API. Each owner also has an independent
paper-trading switch and caps for total exposure, daily loss, open orders, and
active strategies.

Selected account pairs are active immediately and require no administrator approval. Save an exchange API connection and run its read-only `Test` action; the test reads private balances and account-wide open orders across the venue's supported market types but never creates or cancels an order. Selected pair bindings add market metadata and order-book checks. A successful test is valid for 24 hours. Changing credentials resets the connection and its bindings to unverified, and stale or failed market checks disable affected bindings. API secrets are write-only in the browser and encrypted at rest with AES-GCM; neither state APIs nor audit records return the secret values.

The settings page presents one exchange-account workflow and points to the next required action: API credentials, read-only connection test, tradable-currency selection, paper strategy creation, or paper strategy enablement. Internal pair bindings show their own readiness and retain the remaining lifetime of a successful connection test. Workspace readiness is calculated from one batched account/pair/strategy snapshot so adding more users and strategies does not create per-strategy database reads.
