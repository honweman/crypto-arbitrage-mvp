...
This workspace flow provisions and verifies per-user accounts but does not automatically attach them to a live strategy. Existing global MM, Auto Buy/Sell, and arbitrage runners remain separately configured until per-user strategy instances, risk limits, and P/L ownership are explicitly bound to these accounts.
