...
Each registered user gets isolated global API connections plus a project workspace. An API connection is configured once with the exchange, market type, API region, and trade-only credentials; it can be saved and tested before any project exists. Its private balance snapshot appears in monitoring immediately. Projects independently define an asset and quote currency. The server discovers active exchange markets and automatically creates one trading binding for every compatible project, including projects added after the API connection. Upbit supports both Global and Indonesia (`id.Upbit`) API regions, and Bithumb user accounts use API v2.0.
...
New user projects are active immediately and require no administrator approval. Save a global API connection and run its read-only `Test` action; the test reads private balances and account-wide open orders but never creates or cancels an order. Compatible project bindings add market metadata and order-book checks. A successful test is valid for 24 hours. Changing credentials resets the connection and its bindings to unverified, and stale or failed market checks disable affected bindings. API secrets are write-only in the browser and encrypted at rest with AES-GCM; neither state APIs nor audit records return the secret values.

The settings page keeps global API connections separate from project management and points to the next required action: API credentials, read-only connection test, project creation, automatic market binding, paper strategy creation, or paper strategy enablement. Binding rows show their own readiness progress and the remaining lifetime of a successful connection test. Workspace readiness is calculated from one batched project/account/strategy snapshot so adding more users and strategies does not create per-strategy database reads.

Users can create isolated paper strategy instances for Market Maker, Auto Buy/Sell, Spot Grid, DCA, cross-exchange Spot Arbitrage, CEX/DEX Contract Arbitrage, and Polymarket Arbitrage. Each instance is bound to one project and only that owner's verified exchange accounts, with its own order budget, total budget, daily-loss, open-order, slippage, and order-book-age limits. Contract Arbitrage requires at least one spot account and one swap/future account, can require a Hyperliquid, dYdX, or Aster derivative leg, converts each venue's quote currency into the configured common quote, and evaluates depth-adjusted basis, fees, slippage, and supportive funding. Polymarket Arbitrage batches public CLOB books by outcome asset ID and scans same-condition binary complete sets, multi-outcome complete sets, Neg-Risk NO-to-other-YES conversions, and explicitly configured price-event hedges against CEX or DEX spot/perpetual books. Complete-set and Neg-Risk candidates include depth, fees, conversion cost, inventory requirements, condition structure, and augmented-Other review gates. Cross-venue candidates use an expiry/strike/volatility digital-payoff model and Delta hedge estimate, so they are labeled model-relative-value rather than risk-free arbitrage. Both strategy types currently emit paper candidates and leg metrics only; wallet authorization never enables live submission. A strategy can be resumed only when its project is active, its account count and symbols match, credentials remain available in the encrypted vault, withdrawal permission is confirmed disabled, and every selected account has a fresh successful connection test. Credential or market changes, failed/stale connection checks, and project scope changes automatically pause affected instances.
