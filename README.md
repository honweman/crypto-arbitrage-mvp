...
The web page exposes runtime controls for the selected account, `enabled`, `side`, `price_mode`, `price_offset_bps`, `unlimited_total`, `slice_mode`, `total_base`, `total_quote`, the min/max base order size range, randomization, `interval_seconds`, `order_ttl_seconds`, `start_price`, and `stop_price`. The account selector includes every configured spot account plus Binance USDM and Bybit USDT linear-swap accounts. Multiple accounts should use distinct `label` values. These page edits affect the running monitor immediately but do not write back to the base JSON config.

Auto Buy/Sell can also execute Binance USDM and Bybit USDT linear perpetuals. Set `instrument_type` to `perpetual` and use a canonical ccxt symbol such as `BTC/USDT:USDT`. The UI quantities remain underlying base units and are converted to exchange contracts using `contractSize`. Perpetual tasks require a finite target and configured slice size. `position_effect` defaults to `reduce_only`; every live reduce order is checked against the current matching position and sent with `reduceOnly`. Opening or increasing a position requires `max_position_quote`, a positive `risk.max_derivative_leverage`, and a task `leverage` no higher than that global limit. Only one-way position mode is supported; hedge-mode accounts must not be used with this task type.
